...
class ServerlessDynamodbLocal {
    ...
    this.options = _.merge(
      {
        localPath:
          serverless.config &&
          path.join(serverless.config.servicePath, ".dynamodb"),
      },
      options,
    );
    this.provider = "aws";
          ...
          seed: {
            ...
            options: {
              online: {
                shortcut: "o",
                usage:
                  "Will connect to the tables online to do an online seed run",
              },
            },
          },
          start: {
            ...
            options: {
              ...
              cors: {
                shortcut: "c",
                usage:
                  'Enable CORS support (cross-origin resource sharing) for JavaScript. You must provide a comma-separated "allow" list of specific domains. The default setting for -cors is an asterisk (*), which allows public access.',
              },
              inMemory: {
                shortcut: "i",
                usage:
              ...
              migrate: {
                shortcut: "m",
                usage:
                  "After starting dynamodb local, create DynamoDB tables from the current serverless configuration.",
              },
              seed: {
                shortcut: "s",
                usage:
              ...
              heapInitial: {
                usage:
                  'The initial heap size. Specify megabytes, gigabytes or terabytes using m, b, t. E.g., "2m"',
              },
              heapMax: {
                usage:
                  'The maximum heap size. Specify megabytes, gigabytes or terabytes using m, b, t. E.g., "2m"',
              },
              convertEmptyValues: {
                shortcut: "e",
                usage:
  ...
  get port() {
    return _.get(this.config, "start.port", 8000);
  }

  get host() {
    return _.get(this.config, "start.host", "localhost");
  }

  get region() {
    return _.get(this.config, "start.region", "localhost");
  }

  get accessKeyId() {
    return _.get(this.config, "start.accessKeyId", "MOCK_ACCESS_KEY_ID");
  }

  get secretAccessKey() {
    return _.get(this.config, "start.accessKeyId", "MOCK_SECRET_ACCESS_KEY");
  }
  ...
  dynamodbOptions(options) {
    ...
    if (options && options.online) {
      ...
      dynamoOptions = {
        ...
        convertEmptyValues:
          options && options.convertEmptyValues
  ...
  migrateHandler() {
    if (this.shouldExecute()) {
      ...
      this.serverlessLog(
        "Skipping migration: DynamoDB Local is not available for stage: " +
          this.stage,
      );
    }
  }
    ...
    if (this.shouldExecute()) {
      ...
      return BbPromise.each(this.seedSources, source => {
        ...
        const seedPromise = seeder
          ...
          .then(seeds =>
            seeder.writeSeeds(
              dynamodb.doc.batchWrite.bind(dynamodb.doc),
              source.table,
              seeds,
            ),
          );
        const rawSeedPromise = seeder
          .locateSeeds(source.rawsources || [])
          .then(seeds =>
            seeder.writeSeeds(
              dynamodb.raw.batchWriteItem.bind(dynamodb.raw),
              source.table,
              seeds,
            ),
          );
      ...
      this.serverlessLog(
        "Skipping seeding: DynamoDB Local is not available for stage: " +
          this.stage,
      );
    }
  }
  ...
  startHandler() {
    if (this.shouldExecute()) {
      const config =
        (this.service.custom && this.service.custom.dynamodb) || {};
      ...
      if (dbPath) {
        options.dbPath = path.isAbsolute(dbPath)
          ? dbPath
          : path.join(this.serverless.config.servicePath, dbPath);
      }
      ...
      this.serverlessLog(
        "Skipping start: DynamoDB Local is not available for stage: " +
          this.stage,
      );
    }
  }
  ...
  endHandler() {
    if (this.shouldExecute() && !this.options.noStart) {
      ...
      this.serverlessLog(
        "Skipping end: DynamoDB Local is not available for stage: " +
          this.stage,
      );
    }
  }
  ...
  get seedSources() {
    ...
    if (typeof seed === "string") {
      ...
      // if (!seed)
      this.serverlessLog(
        "DynamoDB - No seeding defined. Skipping data seeding.",
      );
      return [];
    }
    const sourcesByCategory = categories.map(
      category => seedConfig[category].sources,
    );
